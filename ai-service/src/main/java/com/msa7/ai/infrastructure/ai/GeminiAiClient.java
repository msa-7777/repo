...
public class GeminiAiClient {
    ...
    public String buildPrompt(OrderResponse order, ProductResponse product, DeliveryResponse delivery, OrderWithDeliveryDto orderWithDelivery, HubRoutePathResponse getRouteInfo) {
        ...
        String routeStr = (delivery.routeRecords() == null || delivery.routeRecords().isEmpty())
                ...
                .map(r -> String.format("[순서:%d, 출발:%s -> 도착:%s, 거리:%dkm, 소요:%d분]", //
                        r.sequence(), delivery.startHubId(), delivery.endHubId(), getRouteInfo.totalDistance(), getRouteInfo.totalDuration()))   //
