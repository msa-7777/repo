...
public interface InternalHubApi {
    ...
    boolean checkHubExists(@Parameter(description = "허브 ID") UUID hubId);

    @Operation(summary = "두 허브 간 최적 경로 조회", description = "출발/도착 허브 간 이동 경로 및 소요시간 조회")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    HubRoutePathResponse getRouteInfo(
            @Parameter(description = "출발 허브 ID") UUID startHubId,
            @Parameter(description = "도착 허브 ID") UUID endHubId
    );
}